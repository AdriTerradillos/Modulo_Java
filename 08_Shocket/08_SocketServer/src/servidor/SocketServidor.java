...
public class SocketServidor {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		//y creará un objeto socket por cada petición.
		...
		//en este caso no lo crearemos nosotros, sino que será el SocketServer quien
		//lo cree cuando acepte una petición de un cliente.
		Socket socketAlCliente = null;
		
		InetSocketAddress direccion = new InetSocketAddress(PUERTO);
		
		//En este caso no podemos hacer la declaración try-with-resources como antes
		//ya que el servidor es un hilo que no para nunca y por cada petición
		//crea un nuevo objeto Socket a partir del objeto ServerSocket, es decir,
		//será el objeto ServerSocket el que nos crerará el objeto Socket por nosostros
		try (ServerSocket serverSocket = new ServerSocket()){			
			...
			//Vamos a llevar la cuenta del número de peticiones que nos llegan
			int peticion = 0;
			...
			while(true){		
				...
				//En este punto, se parara el programa, hasta que entre la petición de 
				//un cliente, y será en ese momento cuando se cree un objeto Socket
				...
				//información, es decir, cuando haga un salida.println(INFORMACION);				
				...
				System.out.println("SERVIDOR: El cálculo de los numeros es: " + resultado);
								
				//Mandamos el resultado al cliente
				salida = new PrintStream(socketAlCliente.getOutputStream());
				salida.println(resultado);	
				
				//Si hemos llegado hasta aquí, cerramos las conexiones
